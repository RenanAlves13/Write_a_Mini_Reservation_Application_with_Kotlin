...
// Task 2: define another version of getPassengerAt but takes an index (Int) as a parameter
//         and return a nullable string
//         enclose the following in its body
        ...
        do {
            ...
            when (role) {
                ...
                "3"      -> {
                    ...
                    attendants.add(attendantName asRole "Flight Attendant") // Task 7 and 8
                    printCrew()
                    roleLoopDone = true
                }
...
fun subMenuManifest() {
    ...
    // Task 4: comment out this block, we are going to use higher order functions instead of an index loop

    /*
    for (index in 0 until totalSeats) {
        if (!isSeatAvailable(index)) {
            // Task 2: call getPassengerAt(index) and append it to seatDesc instead of the placeholder
    ...
    */
    ...
    var lines = passengers.asSequence().mapIndexed { index, it ->
        getSeatDesignatorFromIndex(index) + " " + it
    }.filterIndexed { index, it -> !isSeatAvailable(index) }

    // Task 4: take passenger's names and add seat number
    //var lines = passengers.mapIndexed{index, it ->
     //   getSeatDesignatorFromIndex(index) + " " + it
   //}

    // Task 4: get rid of the empty seats
...
// Task 7 and 8: add an extension function
